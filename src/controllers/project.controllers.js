...
const getProjects = asyncHandler(async (req, res) => {
  const projects = await ProjectMember.aggregate([
    ...
    { $unwind: "$project" },
    { $project: { project: 1, role: 1, _id: 0 } },
  ]);
...
const createProject = asyncHandler(async (req, res) => {
  ...
  await ProjectMember.create({ user: req.user._id, project: project._id, role: UserRolesEnum.ADMIN });
...
const addMembersToProject = asyncHandler(async (req, res) => {
  ...
  const member = await ProjectMember.findOneAndUpdate(
    ...
    { $set: { role }, $setOnInsert: { user: user._id, project: projectId } },
...
const deleteMember = asyncHandler(async (req, res) => {
  const { projectId, userId } = req.params;
  const projectMember = await ProjectMember.findOneAndDelete({ project: projectId, user: userId });
  if (!projectMember) throw new ApiError(404, "Project member not found");
